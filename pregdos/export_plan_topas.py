...
class TopasPlan:
    ...
    def generate(myfield: Field, bm: BeamModel, nominal: bool,
        ...
        lines.append(TopasPlan.build_time_features_string(myfield, bm, nominal, nstat))
    ...
    @staticmethod
    def build_time_features_string(myfield: Field, bm: BeamModel, nominal: bool, nstat: int = int(1e6)) -> str:
        """
        Build the TIME FEATURES section for a Topas file and return as a string.
        """

        n_spots = sum(len(layer.spots) for layer in myfield.layers)
        times = np.zeros(n_spots)
        energies = np.zeros(n_spots)
        espreads = np.zeros(n_spots)
        posx = np.zeros(n_spots)
