...
def test_display_name_drops_the_structure_suffix():
    ...
    assert results.display_scorer_name("DoseGamma_Pacemaker", "Pacemaker") == "DoseGamma"
    assert results.display_scorer_name("DoseEquivNeutron_Fetus", "Fetus") == "DoseEquivNeutron"
...
def test_display_name_does_not_care_what_the_structure_was_called():
    ...
    assert results.display_scorer_name("DoseWater_CTV1_68Gy", "CTV1 68Gy") == "DoseWater"
    assert results.display_scorer_name("DoseGamma_PTV_boost", "PTV-boost") == "DoseGamma"
...
def test_display_name_keeps_a_grid_suffix():
    ...
    assert results.display_scorer_name("DoseEquivNeutron_Grid", "") == "DoseEquivNeutron_Grid"


def test_display_name_survives_a_name_with_no_suffix():
    assert results.display_scorer_name("DoseGamma", "Liver") == "DoseGamma"
...
def test_every_generated_prefix_survives_its_own_round_trip():
    ...
    for prefix in _SCORER_NAME.values():
        ...
        assert results.display_scorer_name(f"{prefix}_Pacemaker", "Pacemaker") == prefix
...
def test_neutron_scorer_is_not_reported_as_ambient_dose_equivalent():
    ...
    assert results.display_quantity("AmbientDoseEquivalent", "neutron", "Sv") == "NeutronDoseEquivalent"
...
def test_an_unfiltered_ambient_scorer_keeps_topas_name():
    ...
    assert results.display_quantity("AmbientDoseEquivalent", "", "Sv") == "AmbientDoseEquivalent"
    assert results.display_quantity("AmbientDoseEquivalent", "photon", "Sv") == "AmbientDoseEquivalent"
...
def test_every_gy_quantity_is_marked_as_physical_dose():
    ...
    assert results.display_quantity("DoseToWater", "", "Gy") == "DoseToWater (physical dose)"
    assert results.display_quantity("DoseToMedium", "proton", "Gy") == "DoseToMedium (physical dose)"
...
def test_marking_covers_every_gy_row_not_just_the_proton_ones():
    ...
    for particle in ("proton", "gamma", ""):
        assert results.display_quantity("DoseToMedium", particle, "Gy").endswith("(physical dose)")


def test_sv_is_not_marked():
    """Already a weighted quantity, and not misread this way."""
    assert "physical" not in results.display_quantity("NeutronDoseEquivalent", "neutron", "Sv")


def test_quantities_in_other_units_are_untouched():
    assert results.display_quantity("EnergyDeposit", "proton", "MeV") == "EnergyDeposit"
...
def test_retired_scorer_prefix_is_mapped_to_the_current_one():
    ...
    assert results.canonical_scorer_name("AmBDose_Pacemaker") == "DoseEquivNeutron_Pacemaker"
    assert results.canonical_scorer_name("AmBDoseNeutron_Fetus") == "DoseEquivNeutron_Fetus"
    assert results.canonical_scorer_name("AmBDose_Grid") == "DoseEquivNeutron_Grid"


def test_current_scorer_names_pass_through():
    assert results.canonical_scorer_name("DoseGamma_Pacemaker") == "DoseGamma_Pacemaker"
    assert results.canonical_scorer_name("DoseEquivNeutron") == "DoseEquivNeutron"


def test_display_name_maps_a_retired_prefix_too():
    assert results.display_scorer_name("AmBDose_Pacemaker", "Pacemaker") == "DoseEquivNeutron"
    assert results.display_scorer_name("AmBDose_Grid", "") == "DoseEquivNeutron_Grid"
...
def test_aliases_only_cover_names_no_longer_generated():
    ...
    assert not (current & set(results._SCORER_ALIASES)), "alias shadows a generated prefix"
    assert set(results._SCORER_ALIASES.values()) <= current, "alias points at a dead name"
